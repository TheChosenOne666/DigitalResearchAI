...
var DB = {
  ...
  memberOrders: [
    ...
    {id:'mo3', user:'chen_data',   level:'专业版 · 年', amount:'¥499',   status:'回调异常', time:'2026-06-30'}
  ],
  renews: [
  ...
  orders: [
    {id:'ORD202608100001', user:'user',       title:'会员缴费 · 专业版年费', amount:1299, channel:'微信支付', status:'待支付',   time:'2026-08-10 10:00'},
    {id:'ORD202607150008', user:'li_research',title:'会员缴费 · 专业版年费', amount:499,  channel:'支付宝',   status:'已支付',   time:'2026-07-15 09:20'},
    {id:'ORD202606300021', user:'chen_data',  title:'会员缴费 · 企业版年费', amount:1299, channel:'微信支付', status:'回调异常', time:'2026-06-30 14:02'}
  ],
  payments: [
  ...
  if(tb){
    tb.innerHTML = DB.tasks.map(function(t){
      var act = t.status==='执行中'
        ...
        : (t.status==='失败' ? link('重试',"retryTask('"+t.id+"')") : '');
...
function saveIngestRule(){ toast('已保存入库规则'); }
...
function rAgent(){
  ...
  if(tb2){
    tb2.innerHTML = DB.agentTasks.map(function(t){
      ...
      else if(t.status==='失败') act = link('重试',"agentTaskRetry('"+t.id+"')") + ' ' + link('日志',"agentTaskLog('"+t.id+"')");
...
function saveBackupPolicy(){ toast('自动备份策略已保存（每日 02:00 全量 + 每 6 小时增量）'); }
function backupNow(){
  if(!needAdmin()) return;
...
function saveKbPermission(){ toast('知识权限规则已保存，公开条目对所有用户可见，私有条目按组织/成员授权'); }
...
function openOrderDetail(id){
  ...
  formModal('订单详情', '<p style="line-height:1.9"><b>'+esc(o.id)+'</b><br>用户：'+esc(o.user)+'<br>商品：'+esc(o.title)+'<br>金额：'+money(o.amount)+'<br>渠道：'+esc(o.channel)+'<br>状态：'+esc(o.status)+' · 创建：'+esc(o.time)+'</p><div class="alert info"><svg width="14" height="14" style="vertical-align:-2px"><use href="#i-alert"></use></svg> 支付流水与发票见用户端「账单查询」（U-17）。</div>', '关闭', null);
}
...
function doPay(){
  ...
  formModal('确认支付',
    ...
    '确认支付', function(){
      ...
      DB.memberOrders[0].status = '已支付';
      rPay(); rBilling(); rOrders(); rMembers(); rDash();
      toast('支付成功：'+pid+' 已回调入账，会员权益已生效');
    });
}
