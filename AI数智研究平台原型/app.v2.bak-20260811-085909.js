...
function createReportTask(){
  ...
  DB.kbEntries.unshift({id:'k'+Date.now(), title:rname, type:'Agent 生成', owner:SESSION.user.name, time:nowStr(), status:'复核中', score:0.8+Math.random()*0.15, scope:'公开'});
...
function doPay(){
  ...
  if(!o){
    o = {id:'ORD'+nowStr().replace(/[-: ]/g,''), user:SESSION.user.account, title:'会员缴费 · 专业版年费', amount:1299, channel:ch, status:'待支付', time:nowStr().slice(0,10)+' '+nowStr().slice(11,16)};
    DB.orders.unshift(o);
  }
  formModal('确认支付',
    '<div class="kpi" style="margin-bottom:12px"><div><div class="lbl">订单号</div><div class="val" style="font-size:18px">'+esc(o.id)+'</div></div></div>'+
