...
import type { OrderStoreService } from '../src/modules/member/order.store.service';
...
let orderStore: OrderStoreService;
let subscriptionStore: SubscriptionStoreService;
let pay: PayService;
...
beforeEach(() => {
  ...
  } as unknown as OrderStoreService;
  subscriptionStore = {
    getSubscriptionSystem: vi.fn().mockResolvedValue(null),
  } as unknown as SubscriptionStoreService;
  pay = new PayService(orderStore, subscriptionStore, new PayChannelRegistry([channel]));
});
...
describe('PayService.handleNotify（入账与幂等）', () => {
  it('正常入账：写成功流水 + 事务入账 + 返回生效区间', async () => {
    ...
    expect(orderStore.settlePayment).toHaveBeenCalledTimes(1);
    expect(orderStore.createPaymentSystem).not.toHaveBeenCalled();
  });
  ...
  it('回调重放（同一流水号）→ 幂等命中，不重复入账', async () => {
    ...
    // 第二次：流水已存在
    (orderStore.findPaymentByTxn as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 'pay1', orderId: 'o1', status: 'SUCCESS' });
    const res = await pay.handleNotify('MOCK', payload);
  ...
  it('回调重放不重复延期：第二次调用不触发 settlePayment', async () => {
    const payload = notify();
    await pay.handleNotify('MOCK', payload);
    const callsAfterFirst = (orderStore.settlePayment as ReturnType<typeof vi.fn>).mock.calls.length;
    (orderStore.findPaymentByTxn as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 'pay1', orderId: 'o1', status: 'SUCCESS' });
    await pay.handleNotify('MOCK', payload);
    expect((orderStore.settlePayment as ReturnType<typeof vi.fn>).mock.calls.length).toBe(callsAfterFirst);
  });

  it('订单已支付 + 新流水号 → 记 FAILED 流水，返回 ORDER_NOT_PENDING', async () => {
    (orderStore.getOrderByNoSystem as ReturnType<typeof vi.fn>).mockResolvedValue(makeOrder({ status: 'PAID' }));
    const res = await pay.handleNotify('MOCK', notify('ORD1', 49900, 'TXN2'));
    expect(res).toEqual({ ok: false, reason: 'ORDER_NOT_PENDING' });
    expect(orderStore.createPaymentSystem).toHaveBeenCalledWith(
      't1',
      expect.objectContaining({ status: 'FAILED', transactionNo: 'TXN2' }),
    );
    expect(orderStore.settlePayment).not.toHaveBeenCalled();
  });
  ...
  it('金额不符 → 记 FAILED 流水，返回 AMOUNT_MISMATCH', async () => {
    ...
    expect(orderStore.createPaymentSystem).toHaveBeenCalled();
    expect(orderStore.settlePayment).not.toHaveBeenCalled();
  });

  it('订单不存在 → 404', async () => {
    (orderStore.getOrderByNoSystem as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  it('并发抢先（settlePayment 返回 false）→ 幂等命中', async () => {
    (orderStore.settlePayment as ReturnType<typeof vi.fn>).mockResolvedValue(false);
  ...
  it('续费：以原到期日为基准累加（不浪费剩余天数）', async () => {
    ...
    (subscriptionStore.getSubscriptionSystem as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 's1',
      userId: 'u1',
      level: 'PRO',
      cycle: 'YEAR',
    ...
    const res = await pay.handleNotify('MOCK', notify());
    const call = (orderStore.settlePayment as ReturnType<typeof vi.fn>).mock.calls[0][0];
  ...
  it('跨等级升级：以当前时间为基准，不继承低等级到期日', async () => {
    ...
    (orderStore.getOrderByNoSystem as ReturnType<typeof vi.fn>).mockResolvedValue(
      makeOrder({
