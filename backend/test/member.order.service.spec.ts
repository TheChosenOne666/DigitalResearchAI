...
let planStore: PlanStoreService;
...
beforeEach(() => {
  ...
  svc = new OrderService(planStore, orderStore, subscriptionStore, new PayChannelRegistry([new MockPayChannel(config)]));
});
...
describe('OrderService.createOrder', () => {
  it('落单并返回对外视图（金额换算 + 中文状态/周期名）', async () => {
    ...
    expect(orderStore.createOrder).toHaveBeenCalledTimes(1);
  });

  it('套餐不存在 → 404', async () => {
    (planStore.getPlan as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
...
describe('OrderService.pay', () => {
  it('待支付订单 → 返回渠道预支付参数并回写 payInfo', async () => {
    ...
    expect(orderStore.attachPayInfo).toHaveBeenCalledTimes(1);
  });

  it('订单已超时 → 置为 CLOSED 并报 409', async () => {
    (orderStore.getOrderByNo as ReturnType<typeof vi.fn>).mockResolvedValue(
    ...
    expect(orderStore.updateOrderStatus).toHaveBeenCalledWith('o1', ['PENDING'], 'CLOSED');
  });

  it('订单已支付 → 409', async () => {
    (orderStore.getOrderByNo as ReturnType<typeof vi.fn>).mockResolvedValue(makeOrder({ status: 'PAID' }));
    await expect(svc.pay('u1', 'ORD202608281200000001')).rejects.toMatchObject({ bizCode: 4002 });
  });
...
describe('OrderService.cancel', () => {
  it('待支付 → 取消成功', async () => {
    (orderStore.getOrderByNo as ReturnType<typeof vi.fn>).mockResolvedValue(makeOrder({ status: 'CANCELLED' }));
...
describe('OrderService.pendingOrder（到期懒续费）', () => {
  it('已有待支付订单 → 直接返回，不重复建单', async () => {
    (orderStore.findPendingOrder as ReturnType<typeof vi.fn>).mockResolvedValue(makeOrder());
  ...
  it('连续包月 + 会员已过期 → 自动生成续费订单（isRenewal）', async () => {
    (subscriptionStore.getSubscription as ReturnType<typeof vi.fn>).mockResolvedValue({
      id: 's1',
      userId: 'u1',
      level: 'PRO',
      cycle: 'MONTHLY',
    ...
    expect(planStore.findPlan).toHaveBeenCalledWith('PRO', 'MONTHLY');
    expect(orderStore.createOrder).toHaveBeenCalledWith(
...
describe('OrderService.list', () => {
  it('分页参数透传并返回总数', async () => {
    ...
    expect(orderStore.listOrders).toHaveBeenCalledWith(
  ...
  it('非法状态值被忽略（不拼入查询条件）', async () => {
    ...
    const call = (orderStore.listOrders as ReturnType<typeof vi.fn>).mock.calls[0][0] as Record<string, unknown>;
